...
What has been built, in the order it was built, with the reasoning it was
built on. This is the history half of what used to be TASKS.md; what is still
open, and what is settled and must not be re-derived, is in
[TASKS.md](TASKS.md). Nothing here is a task.
...
## Card effect vocabulary — landed as `cardScript.ts`

The four boxes below were never ticked and the work is done: see **One-shot
and fixture cards have a vocabulary** further down, and "The last 5 cards" in
TASKS.md for what the vocabulary still cannot say.
...
## The Obszar inventory's neighbours
...
purpose so the card work could land on its own, and both below have since been
built. The first is kept struck through rather than deleted, because a note
that said the opposite for a while is worth leaving where somebody can see it
changed. The two that are still parked — class II and III as two battles, and
Handel — are in TASKS.md.
...
Both of these were written down as gaps left open on purpose, and both have
since been closed. The one that is still open — the house rule that does not
journal itself — is in TASKS.md.
