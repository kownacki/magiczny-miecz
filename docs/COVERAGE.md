...
| 11.2 | except by Łódź, or by field and card effects | ✅ | the Łódź and Pan Trzęsawisk open a `przeprawa`, `crossRing` takes it from wherever the figure stands, and it lands „do Obszaru graniczącego z tym, z którego wyruszyłeś" — `across.ts`, read off the Plansza by the printed names in each column and checked against the two crossings the Instrukcja does name (11.1, 11.5). Where a corner faces two, the player says which |
...
| 11.6 | except by Latarnia | ✅ | as 11.2, across the Lodowy Las |
...
| 17.4 | an item or spell may prevent that loss | ✅ | `shieldUpTo` over the seat's `standing` statuses, rolled in `shieldSaves` |
...
| | rule | status | where |
|---|---|---|---|
| 20.1 | three turns as stone, and the figure swapped for the Kamień card | ✅ | `stone_until_round`, and the one comparison that reads it is `stillStone` — five callers, one strictness. The swap is `figureUrl`: the turn bar, the roster and the Obszar's Gracze shelf all draw a seat through it, so a statue is a statue in all three. **Not** the board map, which draws dots and has no room for a card — deliberate, and noted in `board-map.tsx`. `stone` / `unstone` are the override pair: the console could inflict three rounds and not give them back |
| 20.2 | a stone character keeps no items, gold or friends | ✅ | `turnToStone` — items and gold to the field, friends to the used pile. **The box contradicts itself here**: 12.1's worked example has a Książę picking an Opiekun (Przyjaciel) off the square a stoned Hummit left, which 20.2's own sentence forbids. The numbered rule wins; see the note in `stone.ts` |
| 20.3 | Miecz and Magia are kept but unusable | ✅ | never used — the only moment a statue would swing either is defending an attack, and 20.5 forbids the attack. And never changed, which is the other half of the sentence: „po powrocie… nadal ma taką samą wartość" only holds if nothing moved the numbers in between, so `seatsTargeted` passes a statue over |
| 20.4 | cannot move for three turns | ✅ | `nextSeat`. A statue owed lost turns pays them off *while* it is stone rather than after, so being stoned mid-debt costs three rounds and not three plus the debt — settled by 16.1, which counts a turn as lost by being spent doing nothing. The reasoning is written out in `turnQueue.ts` |
| 20.5 | cannot lose Życie, cannot be targeted; keeps its spells but may not speak them | ✅ | stone is not a legal target: `spendLife` takes nothing from it, so a Karta or an Obszar that sweeps the table passes it by, and `attackSeat`, a Poszukiwacz's raid and a Zaklęcie aimed at one named Postać all refuse out loud (`refuseAgainstStone`), while anything aimed at a *group* — the Władca Gromu, a Burza, a Przesilenie — passes it over instead (`seatsTargeted`), because a Zaklęcie thrown at an Obszar still lands on everybody else standing there. The Zaklęcia are kept — 20.2 takes everything else — and cannot be used until the three turns are up: `refuseWhileHeld` reads the Kamień freeze through `allStatusesOf`, which is the only door casting has and is not turn-gated |
