...
export function DrawnActions({
  ...
  /**
   * Everything below decided once, from the props and the reader — see
   * `drawn-decisions.ts`. Null is a Karta the box does not have.
   */
  const decided = drawnDecisionsFor({
    who,
    card,
    cards,
    resolved,
  ...
  if (!canAct) {
    ...
    return (
      <div className="mt-auto flex flex-col gap-2 border-t border-edge pt-3">
        ...
        {rolls ? (
          <DieTable faces={faces} face={said6?.face ?? null} />
        ) : offered.length > 0 ? (
