...
/**
 * "It is not your turn" is one sentence and four rules.
 *
 * Three of them are exceptions, and every one fails quietly in the direction
...
describe("the seat whose turn it is", () => {
  it("may do anything", () => {
    for (const action of ["roll", "move", "draw", "fight", "end"] as const) {
      expect(mayAct(playing(), active, action)).toBe(true);
    }
  });
  ...
  it("is decided on the index, not on the row", () => {
    ...
    expect(mayAct(playing({ active_seat: 1 }), waiting, "roll")).toBe(true);
    expect(mayAct(playing({ active_seat: 1 }), active, "roll")).toBe(false);
  });
});
...
describe("a seat waiting its turn", () => {
  it("is refused the ordinary actions", () => {
    for (const action of ["roll", "move", "draw", "fight", "end", "fight-done"] as const) {
      expect(mayAct(playing(), waiting, action)).toBe(false);
    }
  });
  ...
  it("may still ask for the moment before the dice (17.7)", () => {
    ...
    expect(mayAct(playing(), waiting, "spell-claim")).toBe(true);
    expect(mayAct(playing(), waiting, "spell-release")).toBe(true);
  });
  ...
  it("may try to flee, because in a duel it is never the attacker who flees (17.6)", () => {
    ...
    expect(mayAct(playing(), waiting, "escape")).toBe(true);
  });

  it("may not end somebody else's turn while somebody is playing", () => {
    expect(mayAct(playing(), waiting, "end")).toBe(false);
  });
});
...
describe("a table with nobody playing", () => {
  ...
  it("lets anybody move it on", () => {
    expect(mayAct(playing({ active_seat: null }), waiting, "end")).toBe(true);
    expect(mayAct(playing({ active_seat: null }), active, "end")).toBe(true);
  });

  it("does not let that become a licence to play out of turn", () => {
    for (const action of ["roll", "move", "draw", "fight"] as const) {
      expect(mayAct(playing({ active_seat: null }), waiting, action)).toBe(false);
    }
  });
  ...
  it("is not the same as a seat index of zero", () => {
    ...
    expect(mayAct(playing({ active_seat: 0 }), waiting, "end")).toBe(false);
  });
});
...
describe("an action nobody recognises", () => {
  it("is refused to a waiting seat like any other", () => {
    ...
    expect(mayAct(playing(), waiting, garbage)).toBe(false);
    expect(mayAct(playing(), waiting, undefined)).toBe(false);
    expect(mayAct(playing(), waiting, null as never)).toBe(false);
  });

  it("reaches the active seat, which is where it gets its 400", () => {
    expect(mayAct(playing(), active, "zjedz-smoka" as never as TurnAction)).toBe(true);
  });
});
