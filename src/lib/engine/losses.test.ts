...
describe("what a loss reaches for", () => {
  it("never takes a trophy when a Przedmiot is asked for", () => {
    ...
    const taken = chooseLosses(pack, { what: "item", chosenBy: "random" }, () => 99);
    expect(taken).not.toContain("t1");
    expect(["i1", "i2"]).toContain(taken![0]);
  });

  it("takes every item when the card says all of them", () => {
    expect(chooseLosses(pack, { what: "all-items" })).toEqual(["i1", "i2"]);
  });

  it("takes friends and spells from their own kinds", () => {
    expect(chooseLosses(pack, { what: "friend", chosenBy: "random" })).toEqual(["f1"]);
    expect(chooseLosses(pack, { what: "spell", chosenBy: "random" })).toEqual(["s1"]);
  });

  it("takes nothing when there is nothing of that kind", () => {
    const empty: Losable[] = [{ id: "t1", cardId: "upior", kind: "trophy" }];
    expect(chooseLosses(empty, { what: "item", chosenBy: "random" })).toEqual([]);
  });
  ...
  it("asks the holder when the card does not say otherwise", () => {
    ...
    expect(chooseLosses(pack, { what: "item" })).toBeNull();
    expect(chooseLosses(pack, { what: "item", chosenBy: "you" })).toBeNull();
  });

  it("takes as many as asked, and never the same one twice", () => {
    const taken = chooseLosses(pack, { what: "item", count: 2, chosenBy: "random" }, () => 0);
    expect(taken).toEqual(["i1", "i2"]);
  });

  it("cannot take more than is there", () => {
    const taken = chooseLosses(pack, { what: "item", count: 9, chosenBy: "random" }, () => 0);
  ...
  it("survives a chooser that hands back nonsense", () => {
    ...
    for (const bad of [-5, 1.7, 1e9, Number.NaN] as const) {
      const taken = chooseLosses(pack, { what: "item", chosenBy: "random" }, () => bad);
...
describe("gold", () => {
  it("takes all of it when no amount is named", () => {
    ...
    expect(goldLost({ what: "gold" }, 7)).toBe(7);
  });

  it("takes the named amount, never more than is there", () => {
    expect(goldLost({ what: "gold", count: 2 }, 7)).toBe(2);
    expect(goldLost({ what: "gold", count: 9 }, 3)).toBe(3);
  });

  it("takes no gold for a loss that is not about gold", () => {
    expect(goldLost({ what: "item" }, 7)).toBe(0);
  });

  it("is not a holding, so it takes no cards", () => {
    expect(chooseLosses(pack, { what: "gold" })).toEqual([]);
  });
});

describe("saying what went", () => {
  it("names the kind, the count and whether it was chance", () => {
    expect(lossTaken({ what: "item" })).toBe("Przedmiot");
    expect(lossTaken({ what: "item", count: 2, chosenBy: "random" })).toBe(
      "2 Przedmiot (losowo)",
    );
    expect(lossTaken({ what: "all-items" })).toBe("wszystkie Przedmioty");
  });
});
...
describe("a whole hand at once (Przesilenie, Władca Czarów)", () => {
  ...
  it("takes every Zaklęcie and asks nobody which", () => {
    ...
    expect(chooseLosses(hand, { what: "all-spells" })).toEqual(["a", "b"]);
  });

  it("leaves everything that is not a Zaklęcie", () => {
    expect(chooseLosses(hand, { what: "all-spells" })).not.toContain("c");
  });

  it("is quiet about a hand that was already empty", () => {
    expect(chooseLosses([hand[2]], { what: "all-spells" })).toEqual([]);
  });
  ...
  it("still asks which, when the card takes only one", () => {
    ...
    expect(chooseLosses(hand, { what: "spell" })).toBeNull();
  });
});
